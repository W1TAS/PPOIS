document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
        ...
        if (data.stage) {
            ...
            if (data.stage && data.stage !== "showdown" && isActivePlayer) {
                ...
                toggleButtons(true);
                document.getElementById("ready-button").style.display = "none"; // Скрываем кнопку готовности
            } else {
                document.getElementById("action-bar").style.display = "none";
                toggleButtons(false);
                // Скрываем кнопку готовности, если стадия не showdown
    ...
    reorderedPlayers.forEach((player, index) => {
        ...
        // Добавляем метку победителя, если winnerName существует, независимо от stage
        if (winnerName) {
